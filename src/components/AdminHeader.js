...
import { AdminMobileMenu } from './Menu/AdminMobileMenuItems';
        ...
        <button className='bg-blue-500 text-white rounded-lg px-2 py-2 lg:mr-[58px]' onClick={() => setModalShareActive(!modalShareActive)}>
          ...
          {modalShareActive && (
            ...
            <motion.div 
              ...
              className='absolute modal-shadow top-12 right-10 lg:right-[58px] z-50 p-4 bg-white h-20 w-fit lg:w-fit whitespace-nowrap rounded-tl-2xl rounded-b-2xl flex justify-center items-center'
            >
        ...
        <div className='lg:hidden flex items-center justify-center'>
          <Menu isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen}/>
            <motion.div variants={sidebarVariants} initial="closed" animate={isMenuOpen ? 'open' : 'closed'} className="fixed inset-0 h-full w-full z-20 bg-blue-500">
              <AdminMobileMenu isMenuOpen={isMenuOpen} />
            </motion.div>
        </div>
      </div>
    </header>
